...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Check if we arrived from another page in the site
  if (sessionStorage.getItem('nextPage')) {
    // Clear the nextPage value
    ...
    setTimeout(() => {
      ...
      // Ensure all scripts run properly
      if (typeof jQuery !== 'undefined') {
  ...
  // Handle Vercel deployments specially
  if (window.location.hostname.includes('vercel.app') || 
      !window.location.hostname.includes('localhost')) {
    // Make absolutely sure the preloader goes away
